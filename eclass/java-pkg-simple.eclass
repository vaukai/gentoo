...
# ${JAVA_MODULE_INFO_OUT}/${JAVA_MODULE_NAME}/versions/${JAVA_MODULE_INFO_RELEASE}
...
# Requires an intermediate jar file to be named as "${JAVA_MODULE_NAME}.jar".
...
java-pkg-simple_src_compile() {
	...
	if [[ ${JAVA_MODULE_INFO_OUT} && ${JAVA_MODULE_NAME} ]]; then
		...
		if [[ ${JAVA_MODULE_INFO_RELEASE} && -z ${JAVA_RELEASE_SRC_DIRS[@]} ]]; then
			...
			JAVA_RELEASE_SRC_DIRS=( ["9"]=${JAVA_MODULE_INFO_OUT}/${JAVA_MODULE_NAME}"/versions/9" )
		fi
	fi
	...
	if [[ -n ${JAVA_RELEASE_SRC_DIRS[@]} ]]; then
		...
		if [[ ! $(find . -name ${JAVA_MODULE_NAME}.jar) ]]; then
