...
MAVEN_PROVIDES="com.google.auto.value:auto-value-annotations:${PV} com.google.auto.value:auto-value:${PV}"
...
DEPEND="
	...
	test? (
		...
		>=dev-java/guava-testlib-33.4.6:0
		dev-java/truth:0
	)
"
...
src_compile() {
	...
	cp="${cp}:$(java-pkg_getjars --build-only checker-framework-qual,escapevelocity)"
	cp="${cp}:$(java-pkg_getjars --build-only error-prone-annotations,guava,incap,javapoet)"
	...
	mkdir target/classes || die "mkdir target/classes"	# still needed for openjdk-8
