...
MAVEN_ID="org.mozilla:rhino:1.7.15"
...
JAVA_TEST_GENTOO_CLASSPATH="
	...
	junit-4,snakeyaml
"
JAVA_TEST_RESOURCE_DIRS="test-res"
...
src_prepare() {
	...
	mkdir res || die
	find src toolsrc -type f ! -name '*.java' | xargs cp --parents -t res || die

	mkdir test-src test-res || die
	find test-src -name 'tests' -exec rm -rf {} + || die
	find testsrc -type f ! -name '*.java' | xargs cp --parents -t test-res || die
