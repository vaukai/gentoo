...
src_prepare() {
	...
	# The java-pkg-simple.eclass compiles all *.java files found in JAVA_SRC_DIR and
	# puts the resulting class files in the jar file.
	# For each of the jar files to be created it needs a separate JAVA_SRC_DIR directory.
...
src_install() {
	# TODO:
	# selectively copy src/etc/*.xsl files
